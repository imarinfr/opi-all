...
#' @examples
#' chooseOpi("Compass")
#' result <- opiInitialise(ip = "192.126.0.1", port = 50000, ip_Monitor = "localhost", port_Monitor = 50001, eye = "left")
#'
...
opiInitialise_for_Compass <- function(ip=NULL, port=NULL, ip_Monitor=NULL, port_Monitor=NULL, eye=NULL) {
    ...
    res <- fromJSON(readLines(env.Compass$socket, n=1))
    return(res)
}
...
#' @examples
#' chooseOpi("Compass")
#' result <- opiPresent(stim = list(x = list(0), y = list(0), t = list(200), w = list(1500), lum = list(20)))
#'
#' @seealso [opiPresent()]
...
if(!exists(env.Compass$socket) || is.null(env.Compass$socket))
    ...
    msg <- list(x = x,y = y,t = t,w = w,lum = lum);
    msg <- toJSON(msg)
    writeLines(msg, env.Compass$socket)
...
if(!exists(env.Compass$socket) || is.null(env.Compass$socket))
    ...
    msg <- list(fixType = fixType,fixCx = fixCx,tracking = tracking);
    msg <- toJSON(msg)
    writeLines(msg, env.Compass$socket)
