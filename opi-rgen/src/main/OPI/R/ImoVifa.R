...
#'  * msg JSON Object with all of the other fields described in @ReturnMsg except 'error'.
#'  * msg$jovp Any messages that the JOVP sent back.
#'
#' @examples
#' chooseOpi("ImoVifa")
#' result <- opiInitialise(ip = "192.126.0.1", port = 50000, ip_Monitor = "localhost", port_Monitor = 50001)
#'
...
opiInitialise_for_ImoVifa <- function(ip=NULL, port=NULL, ip_Monitor=NULL, port_Monitor=NULL) {
    ...
    res <- fromJSON(readLines(env.ImoVifa$socket, n=1))
    return(res)
}
...
#' @param rotation List of angles of rotaion of stimuli (degrees). Only useful if sx != sy specified.
#' @return a list contianing:
#'  * error Empty string for all good, else error messages from ImoVifa.
#'  * msg JSON Object with all of the other fields described in @ReturnMsg except 'error'.
#'  * msg$seen true if seen, false if not.
#'  * msg$time Response time from stimulus onset if button pressed, -1 otherwise (ms).
...
#' @examples
#' chooseOpi("ImoVifa")
#' result <- opiPresent(stim = list(eye = list('left'), type = list('circle'), x = list(0), y = list(0), t = list(200), w = list(1500), lum = list(20), color = list('white'), sx = list(1.72), sy = list(1.72)))
#'
#' @seealso [opiPresent()]
...
if(!exists(env.ImoVifa$socket) || is.null(env.ImoVifa$socket))
    ...
    msg <- list(eye = eye,type = type,x = x,y = y,t = t,w = w,lum = lum,color = color,sx = sx,sy = sy,rotation = rotation);
    msg <- toJSON(msg)
    writeLines(msg, env.ImoVifa$socket)
...
#' @param fixRotation Angles of rotation of fixation target (degrees). Only useful if sx != sy specified.
...
#' @examples
#' chooseOpi("ImoVifa")
#' result <- opiSetup(settings = list(eye = "both", bgLum = 10, bgCol = "white", fixType = "maltese", fixLum = 20, fixCol = "green", fixCx = 0, fixCy = 0, fixSx = 1, fixSy = 1, fixRotation = 0, tracking = 0))
#'
#' @seealso [opiSetup()]
...
if(!exists(env.ImoVifa$socket) || is.null(env.ImoVifa$socket))
    ...
    msg <- list(eye = eye,bgLum = bgLum,bgCol = bgCol,fixType = fixType,fixLum = fixLum,fixCol = fixCol,fixCx = fixCx,fixCy = fixCy,fixSx = fixSx,fixSy = fixSy,fixRotation = fixRotation,tracking = tracking);
    msg <- toJSON(msg)
    writeLines(msg, env.ImoVifa$socket)
...
#'  * msg$isCalibrated 0 eye tracking has not been calibrated, any other value it has.
#'
#' @examples
#' chooseOpi("ImoVifa")
