...
env.PhoneHMD <- vector("list")    # environment for this machine in R
...
opiInitialise_for_PhoneHMD <- function(ip = NULL, port = NULL, ip_Monitor = NULL, port_Monitor = NULL) {
    env.PhoneHMD$socket <<- open_socket(ip_Monitor, port_Monitor)
    msg <- list(ip = ip,port = port,ip_Monitor = ip_Monitor,port_Monitor = port_Monitor);
    msg <- rjson::toJSON(msg)
    writeLines(msg, env.PhoneHMD$socket)

    res <- rjson::fromJSON(readLines(env.PhoneHMD$socket, n=1))
    return(res)
}
...
#' @seealso [opiPresent()]
#'
opiPresent_for_PhoneHMD <- function(stim = list(eye = NULL, type = NULL, x = NULL, y = NULL, t = NULL, w = NULL, lum = NULL, color = NULL, sx = NULL, sy = NULL, rotation = NULL)) {
if(!exists(env.PhoneHMD$socket) || is.null(env.PhoneHMD$socket))
...
#' @seealso [opiSetup()]
#'
opiSetup_for_PhoneHMD <- function(settings = list(eye = NULL, bgLum = NULL, bgCol = NULL, fixType = NULL, fixCx = NULL, fixCy = NULL, fixSx = NULL, fixSy = NULL, fixRotation = NULL)) {
if(!exists(env.PhoneHMD$socket) || is.null(env.PhoneHMD$socket))
    stop("Cannot call opiSetup without an open socket to Monitor. Did you call opiInitialise()?.")

    msg <- list(eye = settings$eye,bgLum = settings$bgLum,bgCol = settings$bgCol,fixType = settings$fixType,fixCx = settings$fixCx,fixCy = settings$fixCy,fixSx = settings$fixSx,fixSy = settings$fixSy,fixRotation = settings$fixRotation);
    msg <- rjson::toJSON(msg)
    writeLines(msg, env.PhoneHMD$socket)
