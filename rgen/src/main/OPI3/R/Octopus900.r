...
#' @usage NULL
#'
#' @param \code{stim} Stimulus to present (a list, see details).
#' @param \code{nextStim} The stimulus to present after stim
#'        (it is not presented, but projector can move to it during response window)
#' @param \code{F310} If \code{F310} is \code{FALSE}, response is taken from internal button.
...
#'        by [\code{opiSetup()}] or [\code{opiInitialize()}]
...
#'
#' @usage NULL
opiPresent_for_Octopus900 <- function(stim, nextStim = NULL) UseMethod("octo900.opiPresent")

setGeneric("opiPresent_for_Octopus900")

#' @rdname opiPresent_for_Octopus900
#' @details
...
#' Implementation of opiPresent.opiTemporalStimulus for the Octopus090 machine.
#'
#' This is for internal use only. Use [opiSetup()] with
#' these Arguments and you will get the Value back.
#'
...
#' Implementation of opiPresent.opiKineticStimulus for the Octopus090 machine.
#'
#' This is for internal use only. Use [opiSetup()] with
#' these Arguments and you will get the Value back.
#'
...
#'
#
# Input paras are the opi_env$O900$* constants
# lum is in cd/m^2 (as per OPI spec) * 100 == .opi_env$O900$BG_{OFF | 1 | 10 | 100 }
# color is .opi_env$O900$MET_COL_{WW | BY | RW | BLUE_WHITE | RED_YELLOW | WHITE_YELLOW }
# fixation is .opi_env$O900$FIX_{RING | CROSS | CENTRE}
...
#'   if(!is.null(oi))
#'     stop("opiInitialize failed")
#'   if(!is.null(opiSetBackground(fixation=.opi_env$O900$FIX_CENTRE)))
#'     stop("opiSetBackground failed")
#'   if(!is.null(opiSetBackground(fixation=.opi_env$O900$FIX_RING, fixIntensity=0)))
#'     stop("opiSetBackground failed")
#'   if(!is.null(opiSetBackground(color=.opi_env$O900$MET_COL_BY)))
#'     stop("opiSetBackground failed")
#'   if(!is.null(opiSetBackground(lum=.opi_env$O900$BG_100, color=.opi_env$O900$MET_COL_RW)))
#'     stop("opiSetBackground failed")
#'   opiClose()
#' }
opiSetup_for_O900 <- function(lum = NA, color = NA, fixation = NA, fixIntensity = NA) {
